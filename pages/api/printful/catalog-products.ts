...
export default async function handler(
  ...
  try {
    ...
    const prr = (body.data ?? []) as PrintfulCatalogProduct[];
    console.log('???')
    console.log(prr)
    console.log('???')
    const products = prr.filter(pro => pro.name.toLowerCase().includes("hoodie") || pro.name.toLowerCase().includes("poster") || pro.name.toLowerCase().includes("jacket") || pro.name.toLowerCase().includes("t-shirt"));
